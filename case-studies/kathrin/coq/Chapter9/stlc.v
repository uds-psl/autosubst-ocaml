...
Definition congr_Base : Base = Base := eq_refl.

Definition congr_Fun {s0 : ty} {s1 : ty} {t0 : ty} {t1 : ty} (H0 : s0 = t0)
  (H1 : s1 = t1) : Fun s0 s1 = Fun t0 t1 :=
  eq_trans (eq_trans eq_refl (ap (fun x => Fun x s1) H0))
...
Definition congr_lam {m_tm : nat} {s0 : ty} {s1 : tm (S m_tm)} {t0 : ty}
  {t1 : tm (S m_tm)} (H0 : s0 = t0) (H1 : s1 = t1) :
  lam m_tm s0 s1 = lam m_tm t0 t1 :=
  eq_trans (eq_trans eq_refl (ap (fun x => lam m_tm x s1) H0))
