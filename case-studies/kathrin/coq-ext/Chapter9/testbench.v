...
Proof.
  ...
  (* rewrite renComp_tm. *)
  (* asimpl. *)
  (* apply subst_morphism. *)
  (* but how do I continue from here.
   Apply subst_morphism is also not promising because of the next lemma, s and t might be different and the equation still holds. *)
  (* Ok it appears I cannot construct an instantiation with two different terms (out of literals, else it's impossible anyways) that does not work. The reason seems to be that anything constructed from literals (variables, var_zero and shift) can be simplified by fsimpl. Then we unfold the definition of subst_tm/ren_tm and can argue about extensional equality of functions again. *)
  Restart.
  (* auto_unfold. *)
  (* asimpl'. *)
  (* simple apply subst_morphism; intros ?. *)
