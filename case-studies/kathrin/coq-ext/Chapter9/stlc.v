...
(* the object that is substituted into should always stay the same.
 (in case it's a literal value like var_zero, fsimpl can simplify it later after we simplified the substitution)
 Therefore, we only parameterize on the subsitution in the morphism lemmas. *)
Lemma ren_morphism {m_tm n_tm:nat} (f g: fin m_tm -> fin n_tm) (Hf: forall x, f x = g x) (s : tm m_tm) :
  ren_tm f s = ren_tm g s.
Proof.
  apply extRen_tm.
  apply Hf.
Qed.

Lemma subst_morphism {m_tm n_tm:nat} (f g : fin m_tm -> tm n_tm) (Hf: forall x, f x = g x) (s: tm m_tm):
  subst_tm f s = subst_tm g s.
Proof.
  apply ext_tm.
  apply Hf.
Qed.

Lemma scons_morphism {m_tm n_tm:nat} (f g : fin m_tm -> tm n_tm) (Hf: forall x, f x = g x) (t: tm n_tm) (x: fin (S m_tm)) :
  scons t f x = scons t g x.
Proof.
  destruct x.
  cbn. apply Hf.
  reflexivity.
...
Ltac asimpl' := repeat (first
                 ...
                 | progress (simple apply subst_morphism; intros ?); idtac "subst_morphism"
                 | progress (simple apply scons_morphism; intros ?); idtac "scons_morphism"
...
Ltac asimpl := repeat try unfold_funcomp;
                ...
                asimpl'; repeat try unfold_funcomp; minimize.
