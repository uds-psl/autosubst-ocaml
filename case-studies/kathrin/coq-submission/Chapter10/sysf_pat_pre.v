...
Inductive ty (n_ty : nat) : Type :=
  ...
  | recty : forall _ : list (prod (nat) (ty n_ty)), ty n_ty.
...
Lemma congr_recty {m_ty : nat} {s0 : list (prod (nat) (ty m_ty))}
  {t0 : list (prod (nat) (ty m_ty))} (H0 : eq s0 t0) :
  eq (recty m_ty s0) (recty m_ty t0).
Proof.
...
Inductive pat (n_ty : nat) : Type :=
  | patvar : forall _ : ty n_ty, pat n_ty
  | patlist : forall _ : list (prod (nat) (pat n_ty)), pat n_ty.
Lemma congr_patvar {m_ty : nat} {s0 : ty m_ty} {t0 : ty m_ty} (H0 : eq s0 t0)
  : eq (patvar m_ty s0) (patvar m_ty t0).
Proof.
exact (eq_trans eq_refl (ap (fun x => patvar m_ty x) H0)).
Qed.
Lemma congr_patlist {m_ty : nat} {s0 : list (prod (nat) (pat m_ty))}
  {t0 : list (prod (nat) (pat m_ty))} (H0 : eq s0 t0) :
  eq (patlist m_ty s0) (patlist m_ty t0).
Proof.
exact (eq_trans eq_refl (ap (fun x => patlist m_ty x) H0)).
Qed.
Fixpoint ren_pat {m_ty : nat} {n_ty : nat}
(xi_ty : forall _ : fin m_ty, fin n_ty) (s : pat m_ty) : pat n_ty :=
  match s with
  | patvar _ s0 => patvar n_ty (ren_ty xi_ty s0)
  | patlist _ s0 =>
      patlist n_ty (list_map (prod_map (fun x => x) (ren_pat xi_ty)) s0)
  end.
Fixpoint subst_pat {m_ty : nat} {n_ty : nat}
(sigma_ty : forall _ : fin m_ty, ty n_ty) (s : pat m_ty) : pat n_ty :=
  match s with
  | patvar _ s0 => patvar n_ty (subst_ty sigma_ty s0)
  | patlist _ s0 =>
      patlist n_ty (list_map (prod_map (fun x => x) (subst_pat sigma_ty)) s0)
  end.
Fixpoint idSubst_pat {m_ty : nat} (sigma_ty : forall _ : fin m_ty, ty m_ty)
(Eq_ty : forall x, eq (sigma_ty x) (var_ty m_ty x)) (s : pat m_ty) :
eq (subst_pat sigma_ty s) s :=
  match s with
...
Fixpoint extRen_pat {m_ty : nat} {n_ty : nat}
(xi_ty : forall _ : fin m_ty, fin n_ty)
(zeta_ty : forall _ : fin m_ty, fin n_ty)
(Eq_ty : forall x, eq (xi_ty x) (zeta_ty x)) (s : pat m_ty) :
eq (ren_pat xi_ty s) (ren_pat zeta_ty s) :=
  match s with
...
Fixpoint ext_pat {m_ty : nat} {n_ty : nat}
(sigma_ty : forall _ : fin m_ty, ty n_ty)
(tau_ty : forall _ : fin m_ty, ty n_ty)
(Eq_ty : forall x, eq (sigma_ty x) (tau_ty x)) (s : pat m_ty) :
eq (subst_pat sigma_ty s) (subst_pat tau_ty s) :=
  match s with
...
(zeta_ty : forall _ : fin k_ty, fin l_ty)
(rho_ty : forall _ : fin m_ty, fin l_ty)
(Eq_ty : forall x, eq (funcomp zeta_ty xi_ty x) (rho_ty x)) (s : pat m_ty) :
eq (ren_pat zeta_ty (ren_pat xi_ty s)) (ren_pat rho_ty s) :=
  match s with
...
(tau_ty : forall _ : fin k_ty, ty l_ty)
(theta_ty : forall _ : fin m_ty, ty l_ty)
(Eq_ty : forall x, eq (funcomp tau_ty xi_ty x) (theta_ty x)) (s : pat m_ty) :
eq (subst_pat tau_ty (ren_pat xi_ty s)) (subst_pat theta_ty s) :=
  match s with
  | patvar _ s0 =>
...
(Eq_ty : forall x, eq (funcomp (ren_ty zeta_ty) sigma_ty x) (theta_ty x))
(s : pat m_ty) :
eq (ren_pat zeta_ty (subst_pat sigma_ty s)) (subst_pat theta_ty s) :=
  match s with
  | patvar _ s0 =>
...
(Eq_ty : forall x, eq (funcomp (subst_ty tau_ty) sigma_ty x) (theta_ty x))
(s : pat m_ty) :
eq (subst_pat tau_ty (subst_pat sigma_ty s)) (subst_pat theta_ty s) :=
  match s with
  | patvar _ s0 =>
...
(sigma_ty : forall _ : fin m_ty, ty n_ty)
(Eq_ty : forall x, eq (funcomp (var_ty n_ty) xi_ty x) (sigma_ty x))
(s : pat m_ty) : eq (ren_pat xi_ty s) (subst_pat sigma_ty s) :=
  match s with
...
Lemma rinstInst_pat {m_ty : nat} {n_ty : nat}
  (xi_ty : forall _ : fin m_ty, fin n_ty) :
  eq (ren_pat xi_ty) (subst_pat (funcomp (var_ty n_ty) xi_ty)).
Proof.
exact (FunctionalExtensionality.functional_extensionality _ _
                (fun x => rinst_inst_pat xi_ty _ (fun n => eq_refl) x)).
Qed.
Lemma instId_pat {m_ty : nat} : eq (subst_pat (var_ty m_ty)) id.
Proof.
exact (FunctionalExtensionality.functional_extensionality _ _
                (fun x => idSubst_pat (var_ty m_ty) (fun n => eq_refl) (id x))).
Qed.
Lemma rinstId_pat {m_ty : nat} : eq (@ren_pat m_ty m_ty id) id.
Proof.
exact (eq_trans (rinstInst_pat (id _)) instId_pat).
Qed.
Lemma renRen_pat {k_ty : nat} {l_ty : nat} {m_ty : nat}
  (xi_ty : forall _ : fin m_ty, fin k_ty)
  (zeta_ty : forall _ : fin k_ty, fin l_ty) (s : pat m_ty) :
  eq (ren_pat zeta_ty (ren_pat xi_ty s)) (ren_pat (funcomp zeta_ty xi_ty) s).
Proof.
...
Lemma renRen'_pat {k_ty : nat} {l_ty : nat} {m_ty : nat}
  (xi_ty : forall _ : fin m_ty, fin k_ty)
  (zeta_ty : forall _ : fin k_ty, fin l_ty) :
  eq (funcomp (ren_pat zeta_ty) (ren_pat xi_ty))
    (ren_pat (funcomp zeta_ty xi_ty)).
Proof.
exact (FunctionalExtensionality.functional_extensionality _ _
...
Lemma compRen_pat {k_ty : nat} {l_ty : nat} {m_ty : nat}
  (sigma_ty : forall _ : fin m_ty, ty k_ty)
  (zeta_ty : forall _ : fin k_ty, fin l_ty) (s : pat m_ty) :
  eq (ren_pat zeta_ty (subst_pat sigma_ty s))
...
Lemma compRen'_pat {k_ty : nat} {l_ty : nat} {m_ty : nat}
  (sigma_ty : forall _ : fin m_ty, ty k_ty)
  (zeta_ty : forall _ : fin k_ty, fin l_ty) :
  eq (funcomp (ren_pat zeta_ty) (subst_pat sigma_ty))
...
Lemma renComp_pat {k_ty : nat} {l_ty : nat} {m_ty : nat}
  (xi_ty : forall _ : fin m_ty, fin k_ty)
  (tau_ty : forall _ : fin k_ty, ty l_ty) (s : pat m_ty) :
  eq (subst_pat tau_ty (ren_pat xi_ty s))
    (subst_pat (funcomp tau_ty xi_ty) s).
Proof.
...
Lemma renComp'_pat {k_ty : nat} {l_ty : nat} {m_ty : nat}
  (xi_ty : forall _ : fin m_ty, fin k_ty)
  (tau_ty : forall _ : fin k_ty, ty l_ty) :
  eq (funcomp (subst_pat tau_ty) (ren_pat xi_ty))
...
Lemma compComp_pat {k_ty : nat} {l_ty : nat} {m_ty : nat}
  (sigma_ty : forall _ : fin m_ty, ty k_ty)
  (tau_ty : forall _ : fin k_ty, ty l_ty) (s : pat m_ty) :
  eq (subst_pat tau_ty (subst_pat sigma_ty s))
...
Lemma compComp'_pat {k_ty : nat} {l_ty : nat} {m_ty : nat}
  (sigma_ty : forall _ : fin m_ty, ty k_ty)
  (tau_ty : forall _ : fin k_ty, ty l_ty) :
  eq (funcomp (subst_pat tau_ty) (subst_pat sigma_ty))
...
Inductive tm (n_ty n_tm : nat) : Type :=
  ...
  | rectm : forall _ : list (prod (nat) (tm n_ty n_tm)), tm n_ty n_tm
  | proj : forall _ : tm n_ty n_tm, forall _ : nat, tm n_ty n_tm
  | letpat :
      forall p : nat,
      forall _ : pat n_ty,
      forall _ : tm n_ty n_tm, forall _ : tm n_ty (plus p n_tm), tm n_ty n_tm.
...
Lemma congr_rectm {m_ty m_tm : nat} {s0 : list (prod (nat) (tm m_ty m_tm))}
  {t0 : list (prod (nat) (tm m_ty m_tm))} (H0 : eq s0 t0) :
  eq (rectm m_ty m_tm s0) (rectm m_ty m_tm t0).
Proof.
exact (eq_trans eq_refl (ap (fun x => rectm m_ty m_tm x) H0)).
Qed.
Lemma congr_proj {m_ty m_tm : nat} {s0 : tm m_ty m_tm} {s1 : nat}
  {t0 : tm m_ty m_tm} {t1 : nat} (H0 : eq s0 t0) (H1 : eq s1 t1) :
  eq (proj m_ty m_tm s0 s1) (proj m_ty m_tm t0 t1).
Proof.
exact (eq_trans
...
Lemma congr_letpat {p : nat} {m_ty m_tm : nat} {s0 : pat m_ty}
  ...
  {t1 : tm m_ty m_tm} {t2 : tm m_ty (plus p m_tm)} (H0 : eq s0 t0)
  (H1 : eq s1 t1) (H2 : eq s2 t2) :
  eq (letpat m_ty m_tm p s0 s1 s2) (letpat m_ty m_tm p t0 t1 t2).
Proof.
exact (eq_trans
                (eq_trans
                   (eq_trans eq_refl
