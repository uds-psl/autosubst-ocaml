...
Inductive tm (n_tm : nat) : Type :=
  ...
  | lam : forall _ : tm (S n_tm), tm n_tm
  | pair : forall _ : tm n_tm, forall _ : tm n_tm, tm n_tm
  | matchpair : forall _ : tm n_tm, forall _ : tm (S (S n_tm)), tm n_tm.
...
Lemma congr_matchpair {m_tm : nat} {s0 : tm m_tm} {s1 : tm (S (S m_tm))}
  {t0 : tm m_tm} {t1 : tm (S (S m_tm))} (H0 : eq s0 t0) (H1 : eq s1 t1) :
  eq (matchpair m_tm s0 s1) (matchpair m_tm t0 t1).
Proof.
exact (eq_trans
                (eq_trans eq_refl (ap (fun x => matchpair m_tm x s1) H0))
