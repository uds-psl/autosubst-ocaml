...
(Eq_vl : forall x, xi_vl x = zeta_vl x) (s : tm m_ty m_vl) :
...
(zeta_ty : fin m_ty -> fin n_ty) (zeta_vl : fin m_vl -> fin n_vl)
(Eq_ty : forall x, xi_ty x = zeta_ty x)
(Eq_vl : forall x, xi_vl x = zeta_vl x) (s : vl m_ty m_vl) :
...
(Eq_vl : forall x, sigma_vl x = tau_vl x) (s : tm m_ty m_vl) :
...
(tau_ty : fin m_ty -> ty n_ty) (tau_vl : fin m_vl -> vl n_ty n_vl)
(Eq_ty : forall x, sigma_ty x = tau_ty x)
(Eq_vl : forall x, sigma_vl x = tau_vl x) (s : vl m_ty m_vl) :
...
(s : tm m_ty m_vl) : ren_tm xi_ty xi_vl s = subst_tm sigma_ty sigma_vl s :=
  match s with
  | app _ _ s0 s1 =>
...
(Eq_ty : forall x, funcomp (var_ty n_ty) xi_ty x = sigma_ty x)
(Eq_vl : forall x, funcomp (var_vl n_ty n_vl) xi_vl x = sigma_vl x)
(s : vl m_ty m_vl) : ren_vl xi_ty xi_vl s = subst_vl sigma_ty sigma_vl s :=
  match s with
  | var_vl _ _ s0 => Eq_vl s0
  | lam _ _ s0 s1 =>
