...
Definition congr_const {m_tm : nat} {s0 : nat} {t0 : nat} (H0 : s0 = t0) :
  const m_tm s0 = const m_tm t0 :=
  eq_trans eq_refl (ap (fun x => const m_tm x) H0).

Definition congr_Plus {m_tm : nat} {s0 : tm m_tm} {s1 : tm m_tm}
  {t0 : tm m_tm} {t1 : tm m_tm} (H0 : s0 = t0) (H1 : s1 = t1) :
  Plus m_tm s0 s1 = Plus m_tm t0 t1 :=
  eq_trans (eq_trans eq_refl (ap (fun x => Plus m_tm x s1) H0))
