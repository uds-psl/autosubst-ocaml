...
Inductive tm (n_tm : nat) : Type :=
  ...
  | const : forall _ : nat, tm n_tm
  | Plus : forall _ : tm n_tm, forall _ : tm n_tm, tm n_tm.
...
Qed.
Lemma congr_const {m_tm : nat} {s0 : nat} {t0 : nat} (H0 : eq s0 t0) :
  eq (const m_tm s0) (const m_tm t0).
Proof.
exact (eq_trans eq_refl (ap (fun x => const m_tm x) H0)).
Qed.
Lemma congr_Plus {m_tm : nat} {s0 : tm m_tm} {s1 : tm m_tm} {t0 : tm m_tm}
  {t1 : tm m_tm} (H0 : eq s0 t0) (H1 : eq s1 t1) :
  eq (Plus m_tm s0 s1) (Plus m_tm t0 t1).
Proof.
