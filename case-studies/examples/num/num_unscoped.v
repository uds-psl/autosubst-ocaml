...
Inductive tm : Type :=
  ...
  | lam : forall _ : tm, tm
  | const : forall _ : nat, tm
  | Plus : forall _ : tm, forall _ : tm, tm.
...
Qed.
Lemma congr_Plus {s0 : tm} {s1 : tm} {t0 : tm} {t1 : tm} (H0 : eq s0 t0)
  (H1 : eq s1 t1) : eq (Plus s0 s1) (Plus t0 t1).
Proof.
exact (eq_trans (eq_trans eq_refl (ap (fun x => Plus x s1) H0))
