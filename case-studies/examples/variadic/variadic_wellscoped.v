...
Definition congr_app {m_tm : nat} {s0 : tm m_tm} {s1 : list (tm m_tm)}
  {t0 : tm m_tm} {t1 : list (tm m_tm)} (H0 : s0 = t0) (H1 : s1 = t1) :
  app m_tm s0 s1 = app m_tm t0 t1 :=
  eq_trans (eq_trans eq_refl (ap (fun x => app m_tm x s1) H0))
    (ap (fun x => app m_tm t0 x) H1).

Definition congr_lam {p : nat} {m_tm : nat} {s0 : tm (plus p m_tm)}
  {t0 : tm (plus p m_tm)} (H0 : s0 = t0) : lam m_tm p s0 = lam m_tm p t0 :=
  eq_trans eq_refl (ap (fun x => lam m_tm p x) H0).
...
(s : tm m_tm) : tm n_tm :=
  ...
  | app _ s0 s1 => app n_tm (ren_tm xi_tm s0) (list_map (ren_tm xi_tm) s1)
...
(s : tm m_tm) : tm n_tm :=
  ...
  | app _ s0 s1 =>
      app n_tm (subst_tm sigma_tm s0) (list_map (subst_tm sigma_tm) s1)
...
subst_tm sigma_tm s = s :=
  ...
  | app _ s0 s1 =>
      congr_app (idSubst_tm sigma_tm Eq_tm s0)
        (list_id (idSubst_tm sigma_tm Eq_tm) s1)
  | lam _ p s0 =>
      congr_lam
...
(s : tm m_tm) : subst_tm sigma_tm s = subst_tm tau_tm s :=
  ...
  | app _ s0 s1 =>
      congr_app (ext_tm sigma_tm tau_tm Eq_tm s0)
        (list_ext (ext_tm sigma_tm tau_tm Eq_tm) s1)
  | lam _ p s0 =>
      congr_lam
