...
Inductive tm (n_tm : nat) : Type :=
  ...
  | app : forall _ : tm n_tm, forall _ : list (tm n_tm), tm n_tm
  | lam : forall p : nat, forall _ : tm (plus p n_tm), tm n_tm.
Lemma congr_app {m_tm : nat} {s0 : tm m_tm} {s1 : list (tm m_tm)}
  {t0 : tm m_tm} {t1 : list (tm m_tm)} (H0 : eq s0 t0) (H1 : eq s1 t1) :
