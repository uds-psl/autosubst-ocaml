...
Inductive tm : Type :=
  ...
  | app : forall _ : tm, forall _ : list (tm), tm
  | lam : forall p : nat, forall _ : tm, tm.
Lemma congr_app {s0 : tm} {s1 : list (tm)} {t0 : tm} {t1 : list (tm)}
  (H0 : eq s0 t0) (H1 : eq s1 t1) : eq (app s0 s1) (app t0 t1).
Proof.
exact (eq_trans (eq_trans eq_refl (ap (fun x => app x s1) H0))
                (ap (fun x => app t0 x) H1)).
Qed.
Lemma congr_lam {p : nat} {s0 : tm} {t0 : tm} (H0 : eq s0 t0) :
  eq (lam p s0) (lam p t0).
Proof.
exact (eq_trans eq_refl (ap (fun x => lam p x) H0)).
Qed.
...
Fixpoint ren_tm (xi_tm : forall _ : nat, nat) (s : tm) : tm :=
  ...
  | app s0 s1 => app (ren_tm xi_tm s0) (list_map (ren_tm xi_tm) s1)
  | lam p s0 => lam p (ren_tm (upRen_list_tm_tm p xi_tm) s0)
  end.
...
Fixpoint subst_tm (sigma_tm : forall _ : nat, tm) (s : tm) : tm :=
  ...
  | app s0 s1 => app (subst_tm sigma_tm s0) (list_map (subst_tm sigma_tm) s1)
  | lam p s0 => lam p (subst_tm (up_list_tm_tm p sigma_tm) s0)
  end.
