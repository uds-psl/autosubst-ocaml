...
Fixpoint fin' (l: list nat) (f: nat -> nat) : Type :=
  match l with
  | [] => False
  | n :: l => (wrapNat (f n)) + (fin' l (fun m => (S n) + (f m)))
  end.

Definition fin (l: list nat) : Type := fin' l (fun n => n).
...
Fixpoint denote' {l: list nat} (f: nat -> nat) (x: fin' l f) {struct l} : nat.
Proof.
  destruct l as [|n l'].
  - destruct x.
  - cbn in x.
    refine (match x with
            | inl (wrapC _) => n
            | inr x' => S (denote' l' (fun m => S n + (f m)) x')
            end).
Defined.

Definition denote {l: list nat} (x: fin l) := denote' (fun n => S n) x.
...
Definition f2 : fin (fl [0;2;3]) := inr (inl (wrapC 2)). (* denotes 2 *)
Definition f3 : fin (fl [0;2;3]) := inr (inr (inl (wrapC 3))). (* denotes 3 *)
