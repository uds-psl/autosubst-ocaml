...
Proof.
exact (eq_trans
                (eq_trans eq_refl (f_equal (fun x => all m_ty x s1) H0))
                (f_equal (fun x => all m_ty t0 x) H1)).
Qed.
...
Proof.
exact (eq_trans
                (eq_trans eq_refl (f_equal (fun x => app m_ty m_tm x s1) H0))
                (f_equal (fun x => app m_ty m_tm t0 x) H1)).
Qed.
...
Proof.
exact (eq_trans
                (eq_trans eq_refl (f_equal (fun x => tapp m_ty m_tm x s1) H0))
                (f_equal (fun x => tapp m_ty m_tm t0 x) H1)).
Qed.
Lemma congr_vt {m_ty m_tm : nat} {s0 : tm m_ty m_tm} {t0 : tm m_ty m_tm}
  (H0 : eq s0 t0) : eq (vt m_ty m_tm s0) (vt m_ty m_tm t0).
Proof.
exact (eq_trans eq_refl (f_equal (fun x => vt m_ty m_tm x) H0)).
Qed.
...
Proof.
exact (eq_trans
                (eq_trans eq_refl (f_equal (fun x => abs m_ty m_tm x s1) H0))
                (f_equal (fun x => abs m_ty m_tm t0 x) H1)).
Qed.
...
Proof.
exact (eq_trans
                (eq_trans eq_refl (f_equal (fun x => tabs m_ty m_tm x s1) H0))
                (f_equal (fun x => tabs m_ty m_tm t0 x) H1)).
Qed.
...
(xi_tm : forall _ : fin m_tm, fin n_tm) (s : tm m_ty m_tm) : tm n_ty n_tm :=
  match s with
  | var_tm _ _ s => var_tm n_ty n_tm (xi_tm s)
  | app _ _ s0 s1 =>
...
eq (ren_tm xi_ty xi_tm s) (ren_tm zeta_ty zeta_tm s) :=
  match s with
  | var_tm _ _ s => ap (var_tm n_ty n_tm) (Eq_tm s)
  | app _ _ s0 s1 =>
...
:=
  match s with
  | var_tm _ _ s => ap (var_tm l_ty l_tm) (Eq_tm s)
  | app _ _ s0 s1 =>
      congr_app
