...
for n in utlc stlc fcbv variadic pi num fol; do
    echo autosubst signatures/${n}.sig -fext -f -s coq -o case-studies/examples-lt813/${n}_wellscoped.v -v lt813
    autosubst signatures/${n}.sig -fext -f -s coq -o case-studies/examples-lt813/${n}_wellscoped.v -v lt813
done

for n in utlc stlc fcbv pi num; do
    echo autosubst signatures/${n}.sig -fext -allfv -f -s ucoq -o case-studies/examples-lt813/${n}_unscoped.v -v lt813
    autosubst signatures/${n}.sig -fext -allfv -f -s ucoq -o case-studies/examples-lt813/${n}_unscoped.v -v lt813
done
...
generate_file() {
    file=$1
    scope=$2
    echo autosubst ${KAT}${file}.sig -o ${KAT}${file}.v -s ${scope} -no-static -f -v lt813
    autosubst ${KAT}${file}.sig -o ${KAT}${file}.v -s ${scope} -no-static -f -v lt813
}
