...
for n in utlc stlc fcbv variadic pi num fol logrel_coq; do
    echo dune exec -- bin/main.exe signatures/${n}.sig -fext -f -s rocq -o case-studies/examples/${n}_wellscoped.v
    dune exec -- bin/main.exe signatures/${n}.sig -fext -f -s rocq -o case-studies/examples/${n}_wellscoped.v
done

# allfv is only supported for unscoped syntax so we only turn it on here
for n in utlc stlc fcbv pi num logrel_coq; do
    echo dune exec -- bin/main.exe signatures/${n}.sig -fext -allfv -f -s urocq -o case-studies/examples/${n}_unscoped.v
    dune exec -- bin/main.exe signatures/${n}.sig -fext -allfv -f -s urocq -o case-studies/examples/${n}_unscoped.v
done

### Prelude test
echo dune exec -- bin/main.exe signatures/prelude.sig -o case-studies/examples/prelude_import.v -f -s urocq -fext -p case-studies/prelude/prelude.v
dune exec -- bin/main.exe signatures/prelude.sig -o case-studies/examples/prelude_import.v -f -s urocq -fext -p case-studies/prelude/prelude.v

### TAPL
# generate code for the tapl exercise
echo dune exec -- bin/main.exe case-studies/tapl-exercise/sysf.sig -o case-studies/tapl-exercise/sysf.v -f -s urocq -fext
dune exec -- bin/main.exe case-studies/tapl-exercise/sysf.sig -o case-studies/tapl-exercise/sysf.v -f -s urocq -fext
