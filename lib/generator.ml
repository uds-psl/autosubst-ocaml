...
let traversal sort scope name no_args ret bargs args var_case_body (sem: string list -> H.cId -> constr_expr list -> constr_expr) funsem : fixpoint_expr t =
  let open H in
  let s = "s" in
  let* cs = constructors sort in
  let* open_x = isOpen sort in
...
let genRenaming sort =
  ...
  traversal sort ms ren_ id (const @@ refApp sort (sty_terms ns)) scopeBinders [xis]
    (fun s -> app1_ (varApp sort ns) (app1_ toVarT s))
    (fun pms c s -> refApp c (sty_terms ns @ List.map ~f:(fun x -> ref_ x) pms @ s))
    map_
...
let genSubstitution sort =
  ...
  traversal sort ms subst_ id (const @@ refApp sort (sty_terms ns)) scopeBinders [sigmas]
    (fun s -> app1_ toVarT s)
    (fun pms c s -> refApp c (sty_terms ns @ List.map ~f:(fun x -> ref_ x) pms @ s)) map_
...
let genIdLemma sort =
  ...
  traversal sort ms idSubst_ (fun s -> app1_ eq_refl_ s) ret (scopeBinders @ beqs) [sigmas; eqs]
    (fun s -> app1_ toVarT s)
    (fun _ c cs -> refApp (congr_ c) cs)
    mapId_
...
let genExtRen sort =
  ...
  let* toVarT = toVar sort eqs in
  traversal sort ms extRen_ (fun s -> app1_ eq_refl_ s) ret (scopeBinders @ beqs) [xis; zetas; eqs]
 ...
 *   let n = VarState.tfresh "n" in
 *   let* hasRen = hasRenamings sort in
 *   let t n = ap_ [ refApp (if hasRen then ren_ sort else subst_ sort) shift
 *                 ; app_ (eq, [n])] in
 *   let defBody = definitionBody sort binder
 *       (matchFin_ (ref_ n) t eq_refl_, t (ref_ n))
 ...
 *                                           refAbs "n" (t (ref_ "n")) ],
 *                   t (ref_ n))) in
 *   pure @@ Definition (upExt_ sort binder, bpms @ scopeBinders @ beq, Some ret, refAbs "n" defBody)
 *
 ...
 *       refApp (subst_ sort) (sty_terms taus @ [s])
 *     ) in
 *   let* toVarT = toVar sort eqs in
 *   traversal sort ms ext_ (fun s -> app_ (eq_refl_, [s])) ret (scopeBinders @ beqs) [sigmas; taus; eqs]
 *     (fun z -> app_ (toVarT, [z]))
 *     (fun _ c cs -> refApp (congr_ c) cs)
 *     mapExt_
 *
 ...
 *   traversal sort ms compRenRen_ (fun s -> app_ (eq_refl_, [s])) ret (scopeBinders @ beqs) [xis; zetas; rhos; eqs]
 *     (fun n -> ap_ [varApp sort ls; app_ (toVarT, [n])])
 *     (fun _ c xs -> refApp (congr_ c) xs)
 *     mapComp_
 *
 ...
 *   let* shift = patternSId sort binder in
 *   let t n = ap_ [refApp (ren_ sort) shift; app_ (eq, [n])] in
 *   let s = eqTrans_
 *       (scons_p_comp' (ref_ n))
 *       (scons_p_congr_ (refAbs "z" (eqTrans_
 *                                     (scons_p_tail' (app_ (xi, [ref_ "z"])))
 *                                     (t (ref_ "z"))))
 *          (refAbs "z" @@ scons_p_head' (ref_ "z"))) in
 *   let defBody = definitionBody sort binder
 *       (matchFin_ (ref_ n) t eq_refl_, t (ref_ n))
 *       (const2 (s, t (ref_ n))) in
 *   pure @@ Definition (up_ren_subst_ sort binder, bpms @ scopeBinders @ beq, Some ret, refAbs "n" defBody)
 *
 ...
 *   traversal sort ms compRenSubst_ (fun s -> app_ (eq_refl_, [s])) ret (scopeBinders @ beqs) [xis; taus; thetas; eqs]
 *     (fun n -> app_ (toVarT, [n]))
 *     (fun _ c xs -> refApp (congr_ c) xs)
 *     mapComp_
 *
 ...
 *                                             @ List.map ~f:(const (refAbs "x" eq_refl_)) pat
 *                                             @ [ app_ (sigma, [n])])))
 *          (ap_ [refApp (ren_ sort) pat; app_ (eq, [n])])) in
 *   let t' n z' = eqTrans_
 *       (refApp (compRenRen_ sort) (pat @ sty_terms zetas'
 *                                  @ List.map2_exn ~f:(>>>) (sty_terms zetas) pat
 *                                  @ List.map ~f:(fun x ->
 ...
 *                       t' (ref_ n) sort')) in
 *   pure @@ Definition (up_subst_ren_ sort binder, bpms @ scopeBinders @ beq, Some ret, refAbs "n" defBody)
 *
 ...
 *                          @ [refApp (subst_ sort) (sty_terms sigmas @ [s])]),
 *       refApp (subst_ sort) (sty_terms thetas @ [s])) in
 *   traversal sort ms compSubstRen_ (fun s -> app_ (eq_refl_, [s])) ret (scopeBinders @ beqs) [sigmas; zetas; thetas; eqs]
 *     (fun n -> app_ (toVarT, [n]))
 *     (fun _ c xs -> refApp (congr_ c) xs)
 *     mapComp_
 *
 ...
 *           (scons_p_congr_ (refAbs "n" (t' (ref_ "n") sort')) hd),
 *                        t' (ref_ n) sort')) in
 *   pure @@ Definition (up_subst_subst_ sort binder, bpms @ scopeBinders @ beq, Some ret, refAbs "n" defBody)
 *
 ...
 *                            @ [refApp (subst_ sort) (sty_terms sigmas @ [s])]),
 *       refApp (subst_ sort) (sty_terms thetas @ [s])) in
 *   traversal sort ms compSubstSubst_ (fun s -> app_ (eq_refl_, [s])) ret (scopeBinders @ beqs) [sigmas; taus; thetas; eqs]
 *     (fun n -> app_ (toVarT, [n]))
 *     (fun _ c xs -> refApp (congr_ c) xs)
 *     mapComp_
 *
 ...
 *                      @ List.map ~f:(const (TermAbs ([BinderName "x"], eq_refl_))) pat
 *                      @ [ app_ (sigma, [n])])))
 *          (ap_ [(refApp (subst_ sort) pat); app_ (eq, [n])])) in
 *   let t' n z' = eqTrans_
 *       (refApp (compSubstSubst_ sort)
 *          (pat @ sty_terms zeta'
 *           @ List.map2_exn ~f:(>>>) shift (sty_terms zeta')
 ...
 *                                    (refApp (subst_ sort) (pat @ [underscore_])))
 *                         ; ref_ "x"]) in
 *   let defBody = definitionBody sort binder
 *       (matchFin_ (ref_ n) t eq_refl_, t (ref_ n))
 *       (fun p z' -> (eqTrans_
 ...
 *       (matchFin_ (ref_ n) t eq_refl_, t (ref_ n))
 *       (const2 (s, t (ref_ n))) in
 *   pure @@ Definition (up_rinstInst_ sort binder, bpms @ scopeBinders @ bxi @ bsigma @ beq, Some ret, refAbs "n" defBody)
 *
 ...
 *     ) in
 *   let* toVarT = toVar sort eqs in
 *   traversal sort ms rinstInst_ (fun s -> app_ (eq_refl_, [s])) ret (scopeBinders @ beqs) [xis; sigmas; eqs]
 *     (fun n -> app_ (toVarT, [n]))
 *     (fun _ c xs -> refApp (congr_ c) xs)
 *     mapExt_
 *
 ...
 *       refApp (subst_ sort) xis'
 *     ) in
 *   let proof = app_ (fext_, [ underscore_; underscore_
 *                               ; refAbs "x"
 ...
 *   let ret = eq_ (
 *       varApp sort ms >>> refApp (subst_ sort) (sty_terms sigmas),
 *       sigma') in
 *   let proof = app_ (fext_, [ underscore_; underscore_
 *                               ; refAbs "x" eq_refl_ ]) in
 ...
 *       substSorts in
 *   let ret = eq_ (refApp (subst_ sort) vars, TermConst Id) in
 *   let proof = app_ (
 *       fext_, [ underscore_; underscore_
 *              ; refAbs "x" (refApp (idSubst_ sort)
 *                             (vars
 *                              @ (List.map ~f:(const (refAbs "n" eq_refl_)) substSorts)
 *                              @ [ app_ (TermConst Id, [ref_ "x"]) ])) ] ) in
 ...
 *       app_Expl (ren_ sort, sty_terms ms
 *                               @ sty_terms ms
 ...
 *   let proof2 = app_ (fext_, [ underscore_; underscore_
 *                                ; refAbs "n" (refApp (renRen_ sort)
 *                                               (sty_terms xis
 *                                                @ sty_terms zetas
 *                                                @ [ ref_ "n" ])) ]) in
 *   pure [ Lemma (renRen_ sort, scopeBinders
 *                              @ [BinderNameType ([s], (refApp sort (sty_terms ms)))],
 ...
 *   let proof' = app_ (fext_, [ underscore_; underscore_
 *                                ; refAbs "n" (refApp (compComp_ sort)
 *                                               (sty_terms sigmas
 *                                                @ sty_terms taus
 *                                                @ [ref_ "n"]))]) in
 *   pure [ Lemma (compComp_ sort, scopeBinders @ [BinderNameType ([s], (refApp sort (sty_terms ms)))],
 *                 ret, ProofExact proof)
